...
public:
	...
	FunctionInfo(std::string& name, const char* type, 
	  unsigned int ProfileGroup = TAU_DEFAULT,
	  const char *ProfileGroupName = "TAU_DEFAULT");
	FunctionInfo(const char* name, std::string& type, 
	  unsigned int ProfileGroup = TAU_DEFAULT,
	  const char *ProfileGroupName = "TAU_DEFAULT");
	FunctionInfo(std::string& name, std::string& type, 
	  unsigned int ProfileGroup = TAU_DEFAULT,
	  const char *ProfileGroupName = "TAU_DEFAULT");
	...
	// The ctor registers with this.
        ...
        static std::vector<FunctionInfo*> FunctionDB[TAU_MAX_THREADS];

#ifdef PROFILE_CALLS
        list < std::pair<double,double> > *ExclInclCallList; 
