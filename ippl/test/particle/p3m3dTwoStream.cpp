...
int main(int argc, char *argv[]){
	...
	//createParticleDistribution(P,"manual", 2, 1, extend_l,extend_r,1.);
#endif
	...
	double total_charge = P->total_charge;
	double k0 = 1./(4.*M_PI);
	...
	//double relative_error = std::sqrt(error)/sqrt(n);
	double relative_V_error = std::sqrt(errorV)/std::sqrt(L2_V_exact);
	//double relative_V_error = std::sqrt(errorV)/sqrt(n);
	...
	//Nx , Nparticles, r_cut, eps,alpha, absolut_err_E, relative_error_E, relative_Etot, absolut_V_err, relative_V_error, relative_Vtot, relVsolid, relVsolid_Fieldenergy, relVSolid_m
	ofs << nr[0] << "," << P->getTotalNum() << "," << interaction_radius << "," << eps << "," << alpha << "," << error << "," << relative_error << "," << fabs(total_E-total_E_exact)/total_E_exact <<  "," << errorV << "," << relative_V_error << "," << fabs(total_V-total_V_exact)/total_V_exact << "," << std::abs(P->potential_energy-U)/U << "," << std::abs(P->field_energy-Ufinite)/Ufinite <<"," <<std::abs(P->integral_phi_m-U)/U << std::endl;
	ofs.close();
#endif
