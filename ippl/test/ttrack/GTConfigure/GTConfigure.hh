...
FMatrix<double,6,6> makeDriftSpace(double length, double slice);
FMatrix<double,6,6> makeQuadropole(double length, double k, double slice);
FMatrix<double,6,6> makeDipole(double length, double alpha, double slice);
FMatrix<double,6,6> makeRotation(double h, double beta);
...
FMatrix<double,6,6> makeDrift(double length, double slice = 1) {
  FMatrix<double,6,6> result;

  int i,j;

  double l = length / slice;
...
FMatrix<double,6,6> makeQuadropole(double length, double k, double slice = 1) {
  FMatrix<double,6,6> result;
  ...
  if(k > 0) {
    ...
    result[0][0] = cos(k * l);
    result[0][1] = k ? sin(k * l) / k : l;
    result[1][0] = -k * sin(k * l);
    result[1][1] = cos(k * l);
    
    result[2][2] = cosh(k * l);
    result[2][3] = k ? sinh(k * l) / k : l;
    result[3][2] = k * sinh(k * l);
    result[3][3] = cosh(k * l);
  } else {
    k = sqrt(-k);

    result[0][0] = cosh(k * l);
    result[0][1] = k ? sinh(k * l) / k : l;
    result[1][0] = k * sinh(k * l);
    result[1][1] = cosh(k * l);

    result[2][2] = cos(k * l);
    result[2][3] = k ? sin(k * l) / k : l;
    result[3][2] = -k * sin(k * l);
    result[3][3] = cos(k * l);
  }
  return result;
}
...
FMatrix<double,6,6> makeRotation(double h, double beta) {
  ...
  result[1][0] = h * tan(beta);
  result[3][2] = -h * tan(beta);
...
FMatrix<double,6,6> makeDipole(double length, double alpha, double slice) {
  ...
  double h = alpha / length;
  double kx = h; // n = 0 no field index
  double ky = 0;
  ...
  result[0][0] = cos(kx * l);
  result[0][1] = 1/kx * sin(kx * l);
  result[1][0] = -kx *sin(kx * l);
  result[1][1] = cos(kx * l);

  result[2][2] = cos(ky * l);
  result[2][3] = (ky == 0 ? l : 1/ky * sin(ky * l));
  result[3][2] = -ky *sin(ky * l);
  result[3][3] = cos(ky * l);

  result[0][5] = h/kx/kx * (1 - cos(kx *l));
  result[1][5] = h/kx * sin(kx * l);

  result[4][0] = h/kx * sin(kx * l);
  result[4][1] = h/kx/kx * (1 - cos(kx *l));

  result[4][5] = h * h /kx /kx/kx *(kx * l - sin(kx * l));
...
bool FSConfigure(FileStream *is, SimCfgData *cfgData, DistrData *distrData)
{
  ...
  cfgData->thetamin = 1e-13;
  cfgData->sigmawidth = 5;
...
#define DBL_EPSILON 2.2e-16
	...
	cfgData->accpx = tk.getReal();
      } else if (tk.isKey("ACCX")) {
	getToken (is, &tk);
	cfgData->accx = tk.getReal();
	...
	cfgData->doCrosssection = true;
      } else if (tk.isKey("THETAMIN")) {
	getToken (is, &tk);
	cfgData->thetamin = tk.getReal();
      } else if (tk.isKey("SIGMAWIDTH")) {
	getToken (is, &tk);
	cfgData->sigmawidth = tk.getReal();
      } else if (tk.isKey("RFVOLTAGE")) {
	getToken (is, &tk);
	cfgData->rfvoltage = tk.getReal();
      } 
	getToken (is, &tk);
    }     
    // update distrData
...

	...
	*/
	...
	newElement.M = makeCavity(2 * Physics::pi * newElement.elemData.v * 1e-3 * cos(newElement.elemData.phi) / newElement.elemData.lambda / simCfg.Ek);

	Inform msg("test ");
	msg << newElement.M << endl;
	
	newElement.elemData.length = 0;
	...
	*/
	...
	newElement.M = makeDrift(newElement.elemData.length, newElement.elemData.slice);
	//	newElement.elemData.length = newElement.elemData.length / newElement.elemData.slice;
	...
	*/
	...
	newElement.M = makeRotation(newElement.elemData.alpha / newElement.elemData.length, newElement.elemData.beta);
	...
	*/
	...
	newElement.M = makeDipole(newElement.elemData.length, newElement.elemData.alpha, newElement.elemData.slice);
	//newElement.elemData.length = newElement.elemData.length / newElement.elemData.slice;
	...
	*/
	...
	newElement.M = makeCompactCombinedFunctionMagnet(newElement.elemData.length / newElement.elemData.alpha, newElement.elemData.alpha / newElement.elemData.slice, newElement.elemData.fieldindex);
	//newElement.elemData.length = newElement.elemData.length / newElement.elemData.slice;
	...
	*/
	...
	newElement.M = makeQuadropole(newElement.elemData.length, newElement.elemData.K0, newElement.elemData.slice);
	//	newElement.elemData.length = newElement.elemData.length / newElement.elemData.slice;
...
ElemData getElementData(FileStream *is, Token tk)
{
  Inform msg("getElementData ");  
    ...
    if (tk.isKey("ROTATION")) {
      newElement.elType = ROTATION;
      get2Token (is, &tk);
      newElement.length=tk.getReal();
      get2Token (is, &tk);
      newElement.alpha=tk.getReal() * Physics::pi / 180;
      get2Token (is, &tk);
      newElement.beta=tk.getReal() * Physics::pi / 180;
      get2Token (is, &tk);
      newElement.a=tk.getReal();
      get2Token (is, &tk);
      newElement.b=tk.getReal();
    ...
    else if (tk.isKey("CCFMAGNET")) {
      newElement.elType = CCFMAGNET;
      get2Token (is, &tk);
      newElement.length=tk.getReal();
      get2Token (is, &tk);
      newElement.slice=tk.getReal();
      ...
      newElement.alpha=tk.getReal() * Physics::pi / 180;
      get2Token (is, &tk);
      newElement.fieldindex=tk.getReal();
      get2Token (is, &tk);
      newElement.a=tk.getReal();
      get2Token (is, &tk);
      newElement.b=tk.getReal();
	...
	newElement.elType = SEXTUPOLE;
      ...
      get2Token (is, &tk);              // LAMBDA
      newElement.lambda=tk.getReal();        
      get2Token (is, &tk);              // a
      newElement.a=tk.getReal();
      get2Token (is, &tk);              // b
      newElement.b=tk.getReal();
