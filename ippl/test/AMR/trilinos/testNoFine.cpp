 ...
 * 
 * @details Test the application of the "no-fine" Laplacian with
 * the proper coarse boundary
 * 
 */
#include <iostream>
...
void test(const param_t& params)
{
    ...
    // Boundary matrix for Dirichlet BC (level 0 to level 1)
    buildCrseBoundaryMatrix(B, maps, ba, dmap, geom, rv, epetra_comm, 1);
    ...
    trilinos2amrex(*rhs[1], result);
    
    if ( params.isWriteYt )
        writeYt(rhs, phi, efield, geom, rr, 1.0, "testNoFine");
}
